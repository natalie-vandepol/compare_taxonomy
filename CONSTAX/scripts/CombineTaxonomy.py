...
# November 15, 2017
#
# Command line: python CombineTaxonomy.py
...
def reformat_RDP(rdp_file):
	...
	output_file = "outputs/otu_taxonomy_rdp_final.txt"
...
def reformat_UTAX(utax_file):
	...
	output_file = "outputs/otu_taxonomy_utax_final.txt"
	output = open(output_file,"w")
	output.write("OTU_ID\tOTU_Score\tKingdom\tK_score\tPhylum\tP_score\tClass\tC_score")
	output.write("\tOrder\tO_score\tFamily\tF_score\tGenus\tG_score\tSpecies\tS_score\n")
	for i, line in enumerate(all_lines):
...
def reformat_SINTAX(sintax_file):
	...
	output_file = "outputs/otu_taxonomy_sintax_final.txt"
	output = open(output_file, "w")
	output.write("OTU_ID\tOTU_Score\tKingdom\tK_score\tPhylum\tP_score\tClass\tC_score")
	output.write("\tOrder\tO_score\tFamily\tF_score\tGenus\tG_score\tSpecies\tS_score\n")
	for i, line in enumerate(all_lines):
...
def count_classifications(filenames):
	...
	for i, file in enumerate(filenames):
		...
		output1 = open("outputs/otu_taxonomy_CountClassified.txt", "w")
	...
	output2 = open("outputs/Classification_Summary.txt", "w")
...
for classifier in three_classifiers:
	file_name = "taxonomy_assignments/otu_taxonomy."+classifier
	try:
		open(file_name,"r")
	except IOError:
