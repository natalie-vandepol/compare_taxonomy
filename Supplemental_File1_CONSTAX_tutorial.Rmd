...
This zipped directory contains everything needed to run the tool. The ITS1-BC dataset has been included for users to test the tool. Users should place a copy of their own datasets within the *otus* directory.
...
Will require installation of Apache Ant as detailed in the *README.txt* inside the CONSTAX directory.
...
Download USEARCH8 (for UTAX) and USEARCH9/USEARCH10 (for SINTAX). Make sure to select the appropriate version for your operation system (MacOS or Linux). Place USEARCH binaries inside CONSTAX directory.
...
Each run of *constax.sh* will create three directories in the CONSTAX directory: *training_files*, *taxonomy_asssignments*, *outputs*. 
...
Please see descriptions of all custom python scripts implemented in CONSTAX. We have included these descriptions in anticipation that users would like to use CONSTAX with other reference databases. All of the scripts described below can be found within the *scripts* directory. 
...
#### *FormatBLAST.py*
We have also included a python script that formats BLAST output to match the format of CONSTAX output (when used with Qiime's *parallel_assign_taxonomy.py*). Although we do not recommend use of BLAST for taxonomy assignment of ribosomal sequences. This script is NOT automated through *constax.sh*. 
